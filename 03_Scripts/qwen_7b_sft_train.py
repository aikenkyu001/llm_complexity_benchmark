...
output_dir = "./07_Finetune/qwen_7b_finetune_results"
...
model = AutoModelForCausalLM.from_pretrained(
    ...
    device_map="auto", # CPU offloading will happen if VRAM is low
    trust_remote_code=True,
)
model.config.use_cache = False
model.gradient_checkpointing_enable() # Critical for 7B on limited VRAM
...
# 6. SFTConfig (Optimized for 7B)
sft_config = SFTConfig(
    output_dir=output_dir,
    num_train_epochs=5, 
    per_device_train_batch_size=1,
    gradient_accumulation_steps=32, # Increase to keep stable global batch size
    optim="paged_adamw_32bit", # Use paged optimizer for better VRAM handling
    save_steps=25,
    logging_steps=1,
...
# 8. Train
print(f"Starting Qwen 2.5-Coder-7B Fine-tuning...")
trainer.train()
...
print(f"Training complete. Qwen 7B Adapter saved to {final_save_path}")
